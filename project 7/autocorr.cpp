...
int main(int argc, char *argv[])
{
	...
	if (me == BOSS)
	{
		for (int dst = 0; dst < NumCpus; dst++)
		{
			...
			MPI_Send(&BigSignal[dst * PPSize], NUMELEMENTS, MPI_FLOAT, dst, TAG_SCATTER, MPI_COMM_WORLD);
		}
	}
	else
	{
		MPI_Recv(PPSignal, NUMELEMENTS, MPI_FLOAT, BOSS, TAG_SCATTER, MPI_COMM_WORLD, &status);
	}
	...
	{
		MPI_Send(PPSums, NUMELEMENTS, MPI_FLOAT, BOSS, TAG_GATHER, MPI_COMM_WORLD);
	}
	...
	if (me == BOSS)
	{
		...
		for (int src = 0; src < NumCpus; src++)
		{
			...
			MPI_Recv(tmpSums, NUMELEMENTS, MPI_FLOAT, src, TAG_GATHER, MPI_COMM_WORLD, &status);
			for (int s = 0; s < MAXSHIFTS; s++)
				BigSums[s] += tmpSums[s];
		}
	}
